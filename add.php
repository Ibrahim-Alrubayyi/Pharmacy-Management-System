...
if (!$connection) {
    ...
    if ('addManager' == $action) {
        ...
        $age                = $_REQUEST['age'] ?? '';
        $date_of_employment = $_REQUEST['date_of_employment'] ?? '';
        ...
        if ($fname && $lname && $lname && $phone && $password) {
            ...
            $query        = "INSERT INTO managers(fname,lname,email,phone,age,date_of_employment,salary,id_number,password) VALUES ('{$fname}','$lname','$email','$phone','$age','$date_of_employment','$salary','$id_number','$hashPassword')";
        ...
        if ($fname && $lname && $lname && $phone) {
            $query = "UPDATE managers SET fname='{$fname}', lname='{$lname}', email='$email', phone='$phone',age='$age',date_of_employment='$date_of_employment',salary='$salary',id_number='$id_number' WHERE id='{$id}'";
            mysqli_query($connection, $query);
            header("location:index.php?id=allManager");
        }
        ...
        if ($fname && $lname && $lname && $phone && $password) {
            $hashPassword = password_hash($password, PASSWORD_BCRYPT);
            $query        = "INSERT INTO pharmacists(fname,lname,email,phone,age,date_of_employment,salary,id_number,password) VALUES ('{$fname}','$lname','$email','$phone','$age','$date_of_employment','$salary','$id_number','$hashPassword')";
            mysqli_query($connection, $query);
        ...
        if ($fname && $lname && $lname && $phone && $password) {
            $hashPassword = password_hash($password, PASSWORD_BCRYPT);
            $query        = "INSERT INTO salesmans(fname,lname,email,phone,age,date_of_employment,salary,id_number,password) VALUES ('{$fname}','$lname','$email','$phone','$age','$date_of_employment','$salary','$id_number','$hashPassword')";
            mysqli_query($connection, $query);
